...
!  r  = alpha*beta*r_root*Lr     ! Radius at root, cm. (ND)
 r  = alpha*beta*r_root     ! Radius at root, cm. (DIMENSIONAL)
  A  = pi*r**2.0           ! Cross-sectional area, cm^2.
  l = lrr*r
    !! This is from Chambers 2020, add additional parameters
    !! to sor06 script to pass in lrr1 and lrr2
...
!*                                                                         *
!***************************************************************************
subroutine impedance (tmstps,Period,ff1,ff2,ff3,rho,mu,r_root,r_min,y_xt,Lr,Fr2,q,g,par1_in,par2_in,lrr,Z0,term_ID,Z_flag,vaso)
implicit none
...
real(lng)                    :: r_root_new,r_min_new,lrr_new
...
!  character (len=40), dimension(nbuf) :: buffer  ! Temporary strings
  ...
  localmax = 0
  trm_rst = Z0!0.1!0.05     ! Terminal resistance could be (nb_terms*resist)
   ...
   beta_max = 0
    ...
    r_root_new = vaso*r_root
    r_min_new  = vaso*r_min
    lrr_new    = lrr/vaso
    write(*,*) 'New root, min, lrr:', r_root_new, r_min_new, lrr_new
    if (Z_flag==1) then
      ! Compute the impedance at the root of the structured tree.
      Z_om =comp_imp (tmstps,Omega,trm_rst,ff1,ff2,ff3,rho,mu,r_root_new,r_min_new,Lr,Fr2,q,g,par1_in,par2_in,lrr_new)
      ...
      Z_hat = Z_om*q/rho/g/Lr
      Y_hat = 1.0/Z_hat
        PeriodND = Period!*q/(Lr**3)
      y_xt   = real(IFFT(bitreverse(FFTshift(Y_hat/PeriodND))),lng)!*rho*g*Lr/q

     ! WANT TO USE term_ID TO PRINT FILES
     write(*,*) "Ves:",term_ID,"Alpha max",alpha_max,"beta max",beta_max,"Z0ND",Z_hat(tmstps/2+1),"Z0 D",Z_om(tmstps/2+1)
     write(*,*) "Zc:",real(Z_hat(tmstps/2+1))+real(Z_hat(tmstps/2+2))+real(Z_hat(tmstps/2+3))+real(Z_hat(tmstps/2+4)) &
                        +real(Z_hat(tmstps/2+5))
      write(term_name,'(I3)') term_ID
    ...
    else ! DONT RECOMPUTE IMPEDANCE IF Z_FLAG IS NOT 1
              fn = "STnetwork_" // adjustl(trim(term_name))
              open (f1, file=fn)
                  read(f1,*) count
                  read(f1,*) alpha_max
