...
extern "C" void impedance_driver_(int *tmstps, double *Period,
				  ...
				  double *rho, double *mu, 
                                  ...
                                  double *par1, double *par2, double *lrr, double *Z_trm, int *term_ID, int *Z_flag, double *vaso);
...
Tube :: Tube (double Length,
              ...
              int terminal_id, int Z_flag, double vaso):
	L(Length),
	rtop(topradius),
	rbot(botradius),
	daughters(daughters),
    ...
    vaso(vaso)
{
  ...
  y	      = new double[tmstps];
    ...
    // Define nondimensional root impedance
...
//          fprintf(stdout,"\n");
      impedance_driver_(&tmstps,&Period,&fa1ND,&fa2ND,&fa3ND,&rho,&mu_pl,&rbot,&rmin,y,&Lr,&Fr2,&q,&g,&par1,&par2,&lrr,&Z0ND,&term_ID,&Z_flag,&vaso);
      // MJC: Use dimensional period
//      impedance_driver_(&tmstps,&Tper,&fa1ND,&fa2ND,&fa3ND,&rho,&mu_pl,&rbot,&rmin,y,&Lr,&Fr2,&q,&g,&par1,&par2,&lrr,&Z0ND,&term_ID,&Z_flag);
  ...
  for (int i=0; i<N; i++)
  {
    //fprintf (fd, "%13.10f %13.10f %15.10f\n",
      ...
      fprintf (fd, "%13.10f %13.10f %15.10f %15.10f %15.10f %17.10f\n",
               //t*Lr3/q, (i+offset)*h*Lr, (P(i,Anew[i])+p0)*rho*g*Lr/conv, (Qnew[i]*q)/(Anew[i]*Lr2), 2*sqrt((Anew[i]*Lr2)/M_PI), c(i, Anew[i])*10);
...
//    if (isnan(pold)) fprintf(stdout,"B: Nan.  %10.5lf     %10.5lf      %10.5lf\n",fr[i],A,A0[i]);
  return pold;
}
...
double Tube :: Bh (int i, double A, int WM)
{
   int ip1 = i+1;
    ...
    // test for NaN
//    if (isnan(pold)) fprintf(stdout,"Bh: Nan.  %10.5lf     %10.5lf      %10.5lf\n",frh[ip1],A,A0h[ip1]);
   return pold;
}
...
//      if (i==0)//(Vnew>c_tmp)
//      fprintf(stdout,"Rtop: %lf C:%lf U:%lf\n",rtop,c_tmp,Vnew);
  }
  return (minimum);
}
...
//  if(k==2) return(F(Q,A) + G(A,90.0) ((j==0)?dBdx1(i,A,WM):dBdx1h(i,A,WM))); //With Gravity
...
//    return ((F(Q,A)+G(A,90.0)) - A*dPdx1(i,A,WM)/Fr2)/(-Q/A + c(i,A,WM));
}
...
void Tube :: bound_right (int qLnb, double k, double theta, double t, int WM)
{
  ...
  // (using subroutine zero_1d in tools.C.
    ...
    // ADD NEWTON PARAMETERS w/ LINE SEARCH HERE
        ...
        double alam, alamin, a, b, den, disc, f2, rhs1, rhs2, stpmax, slope, test, tmplam;
    double grad,pln,xold,fold;
        double alam2 = 0.0;
        double d;
        double ALF = 1.0e-4;
...
//    ok  = zero_1d (&xr, f, df, 1.0e-12);
      ...
      // Or use a line-search
      stpmax = stpmaxMAX;
      grad = f*df;
      pln  = -f/df;
      slope = grad*pln;
      
      test = abs(pln)/fmax(abs(xr),1.0);
      alamin = tolx/test;
      f2 = 0.0;
      SSE_old = 0.5*f*f;
      xold    = xr;
      ...
      while (!armijo_flag) {
          if (alam<1.0) fprintf(stdout,"Step Length: %lf\n",alam);
          xr = xold + alam*pln;
          f  = xr + cst + y[0]*k*(P(N,xr,WM))/(cR-uR);
          SSE_new = 0.5*f*f;
          
          if (alam<alamin) armijo_flag = true;
          else if (SSE_new <= SSE_old +ALF*alam*slope) armijo_flag=true;
          else
          {
              if (alam == 1.0)
...
//              else fprintf(stdout,"Res passed: %12.12lf\n",test);
...
//    fprintf(stdout,"t= %lf\n",t/10.0);
    ...
    for (int i=0; i<nbrves; i++)
   { // COME BACK HERE
//       fprintf(stdout,"at vessel %d\n",i);
            if (Arteries[i] ->rmin > 0)
          {
//              fprintf(stdout,"IN BOUND RIGHT\n");
